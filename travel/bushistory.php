...
$sql = "SELECT * FROM book_bus where c_id = $id";
$result = $link->query($sql);
echo "<center>";
echo "<table border=4 width=1000 height=100>";
echo "<tr> <th> CUSTOMER ID</th> <th> BUS ID</th><th> BOOKING ID</th>";
if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
        echo " <font size=+3 color=blue> <tr style='font-weight:bold'> <td> " . $row["c_id"]."</td>" ." <td> " . $row["b_id"]. "</td>" ." <td> " . $row["book_id"]."</td>";
           }
